...
def sort_holidays():
    ...
    with open(f"{main_path}\\ui_birthdays\\data\\holidays.txt", "r", encoding="utf-8") as before:
        not_sorted = before.readlines()
        new = sorted(not_sorted, key=lambda x: x[0])

    with open(f"{main_path}\\ui_birthdays\\data\\holidays.txt", "w", encoding="utf-8") as after:
        for line in new:
            after.write(line)
...
def data_holidays():
    ...
    with open(f"{main_path}\\ui_birthdays\\data\\holidays.txt", "r", encoding="utf-8") as f:
        lines = f.readlines()
        for line in lines:
