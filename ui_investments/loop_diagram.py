...
class SmartChart(QtChart.QChart):
    ...
    def __explode(self, slice_, is_hovered):
        ...
        if is_hovered:
            ...
            """
            self.__inner.setPieEndAngle(0)
            при увеличении показателя облсть смещается вправо, то есть
            количество места уменьшается
            """
            ...
            self.__inner.setPieStartAngle(start)
            self.__inner.setPieEndAngle(start + 360)
    ...
    def __get_secondary_color(hexcode):
        ...
        :return: QtGui.QColor
        """
        from ui_investments.dataType import color
...
class Example(QtWidgets.QWidget):
    def __init__(self, datas, parent=None):
        ...
        for data in datas:
            self.chart.add_slice(data.name, data.value, data.color)
