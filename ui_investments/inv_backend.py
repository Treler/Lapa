...
from ui_investments.loop_diagram import Data
...
def create_loop_diagram(exchange=""):
    ...
    with open('ui_investments\\loop_digram_colors.txt') as d:
        lines = d.readlines()

    with open('Scripts\\Investments\\json_data.json') as f:
        templates = load(f)
...
def insert_data_to_all_tables(model, table):
    ...
    heads = ("Coin", "Name", "Current price", "Staked", "Earn", "Total",
                     "Avg purchase price", "Cap", "PNL (B/S)", "PNL (B/S/S)")
    model.setHorizontalHeaderLabels(heads)

    databases = ("binance", )
    with open("Scripts\\Investments\\json_data.json", "r", encoding="utf-8") as f1:
